...
from parser.utils import ordered_load
...
class InvoiceTemplate(OrderedDict):
    ...
    def extract(self, optimized_str):
        ...
        if 'regex' in self:
            for ex in self['regex']:
                match = re.search(ex, optimized_str)
                if match:
                    for field, value in match.groupdict().items():
                        output[field] = value

        if 'address' in self:
            start = re.search(self['address']['start'], optimized_str)
            end = re.search(self['address']['end'], optimized_str)
            if not start or not end:
                logger.warning('no lines found - start %s, end %s', start, end)
                return
            extracted_content = optimized_str[start.end():end.start()]
            terminate_at = extracted_content.index(self['address']['terminate_at'])
            address = ''
            for line in re.split(self.options['line_separator'], extracted_content):
                address = address + ' ' + line[0:terminate_at].strip()
